  ...
  return (
    <aside className="w-[240px] min-h-screen bg-white border-r border-gray-100 flex flex-col shadow-soft">
      ...
      <div className="px-5 py-5">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[10px] text-gray-400 leading-tight">Canne connectée</p>
          </div>
        </div>
      </div>
      ...
      <nav className="flex-1 px-3 py-3 space-y-0.5 overflow-y-auto">
        <p className="text-[10px] font-semibold text-gray-300 uppercase tracking-widest px-3 mb-2">Suivi</p>
        {MENU_PRINCIPAL.map(({ to, label, icon, badge }) => (
          <NavItem key={to} to={to} label={label} icon={icon} hasBadge={badge} />
        ))}

        <div className="divider" />
        <p className="text-[10px] font-semibold text-gray-300 uppercase tracking-widest px-3 mb-2">Compte</p>
        {MENU_COMPTE.map(({ to, label, icon, badge }) => (
          <NavItem key={to} to={to} label={label} icon={icon} hasBadge={badge} />
        ))}
        ...
        {utilisateur?.role === 'admin' && (
          <>
            <div className="divider" />
            <p className="text-[10px] font-semibold text-gray-300 uppercase tracking-widest px-3 mb-2">Admin</p>
            <NavItem to="/admin" label="Administration" icon={<IconAdmin />} />
          </>
        )}
      </nav>
      ...
      <div className="mx-3 mb-3">
        <div className="relative rounded-2xl overflow-hidden h-[110px]"
          ...
          <svg viewBox="0 0 220 80" className="absolute bottom-0 left-0 w-full">
            ...
            <rect x="95"  y="52" width="4"  height="20" fill="#1a3a2d"/>
            <ellipse cx="97"  cy="49" rx="9"  ry="11" fill="#2d6a4f" opacity="0.8"/>
          </svg>
          {/* Slogan */}
      ...
      <div className="px-3 pb-4 border-t border-gray-100 pt-3">
        <div className="flex items-center gap-3 px-2 py-2 rounded-xl hover:bg-cream-100 transition-colors">
          <Avatar nom={utilisateur?.nom} prenom={utilisateur?.prenom} size="sm" />
